...
// Loan payment calculator - compares monthly vs bi-monthly payment plans
...
// monthrate = monthly interest rate (annual % ÷ 100 ÷ 12)
// growthfactor = (1+r)^n via pow() - the compounding term in the EMI formula
// bi-monthly version halves the rate (birate) and doubles the payment count (binumpay)	
...
//if you want to tuen this off you use cout.unsetf(ios::fixed)
// setw(n) sets width for the NEXT output only - must be repeated every time
// left/right control alignment within that width, and PERSIST until changed
// (unlike setw, which resets after each use)
